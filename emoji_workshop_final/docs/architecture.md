...
过滤停用词（约 40 个常见无意义词）
       ↓
对每个关键词执行 SQL LIKE 匹配数据库标签名
       ↓
COUNT(DISTINCT tag_id) 统计每张图片命中的标签数
       ↓
按命中数降序排列，取 Top-K
       ↓
若无任何匹配，返回最近导入的 Top-K 作为兜底
```

核心 SQL：

```sql
SELECT i.id, i.name, ..., COUNT(DISTINCT t.id) AS match_count
FROM images i
JOIN image_tags it ON i.id = it.image_id
JOIN tags t       ON it.tag_id = t.id
WHERE t.name LIKE '%keyword1%' OR t.name LIKE '%keyword2%' OR ...
GROUP BY i.id
ORDER BY match_count DESC, i.id DESC
LIMIT top_k
```

算法复杂度：O(N·K)，其中 N 为图片数，K 为关键词数。对于千级图片库，查询耗时通常 < 10ms，无需异步化。
